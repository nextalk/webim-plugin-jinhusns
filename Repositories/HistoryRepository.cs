...
namespace Spacebuilder.Webim
{
    ...
    public class HistoryRepository : Repository<HistoryEntity>, IHistoryRepository
    {
        ...
        {
            CreateDAO().Execute("UPDATE spb_Webim_Histories SET fromdel = 1 Where fromuser = @0 and touser = @1 and type = 'unicast'", uid.ToString(), with);
            CreateDAO().Execute("UPDATE spb_Webim_Histories SET todel = 1 Where touser = @0 and fromuser = @1 and type = 'unicast'", uid.ToString(), with);
